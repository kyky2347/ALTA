...
- **看清线索：**区分实时、近期和过期证据；旧研究可以查看，但不会被当成新机会推荐。
- **跟上进展：**中英文运行简报、聚焦的机会卡片、Agent 交接与可搜索的决策历史。
- **保留上下文：**断网或响应损坏时保留有效数据；迟到的轮询不能覆盖已确认的操作结果。
...
遇到无效响应或断连时，控制台会保留上次有效快照；迟到的轮询不会覆盖已确认的
操作结果。缓存中的机会新鲜度也会在每次读取时重新检查。验证范围与剩余限制见
[本地可靠性审查](docs/audits/console-reliability.md)。
...
版本基线仍为 `0.26.0`；后续改动记录在[变更日志](CHANGELOG.md)的 Unreleased
部分。9 月 9 日的本地审查验证了：

| 自动化检查                                                      | 浏览器与联调检查                                           |
| --------------------------------------------------------------- | ---------------------------------------------------------- |
| **644 项通过：**201 Node · 386 运行时 · 38 模拟盘边界 · 19 前端 | Chromium 下 **84** 组页面、语言和视口组合                  |
| TypeScript、Lint、格式检查与生产构建通过                        | **7** 条贯穿 PostgreSQL、Python 与 Node 网关的鉴权读取接口 |
| 恢复与证据新鲜度回归覆盖                                        | 断网重连、响应损坏、迟到轮询和有界安全会话重试             |

真实服务联调仅执行读取，自主研究与 Tiger 均未启用；临时服务和测试数据库随后已
停止。这些结果不证明券商当前可连接、多日持续可用、真实断电可恢复、所有浏览器
兼容，也不证明获得 Alpha。[可靠性审查](docs/audits/console-reliability.md)记录
验证限制，[发布安全审查](docs/audits/security-audit.md)记录敏感信息扫描范围。
