...
#[tokio::test]
async fn alta_agent_roles_add_portable_collaboration_tools_without_replacing_native_tools() {
    let plan = probe(|turn| {
        set_feature(turn, Feature::MultiAgentV2, /*enabled*/ true);
        update_config(turn, |config| {
            config.agent_roles.insert(
                "alta_deepseek_test".to_string(),
                AgentRoleConfig {
                    description: Some("ALTA DeepSeek test role".to_string()),
                    config_file: None,
    ...
    let functions = plan.namespace_function_names(MULTI_AGENT_V2_NAMESPACE);
    for tool_name in ["spawn_agent", "send_message", "followup_task"] {
        assert!(
            functions.iter().any(|name| name == tool_name),
            "expected {tool_name} in the collaboration namespace: {functions:?}"
        );
    }
    plan.assert_visible_contains(&[
        "alta_spawn_model_agent",
        "alta_send_model_message",
        "alta_followup_model_task",
    ]);
    plan.assert_registered_contains(&[
        "alta_spawn_model_agent",
        "alta_send_model_message",
    ...
    for tool_name in [
        ...
        let ToolSpec::Function(tool) = plan.visible_spec(tool_name) else {
            panic!("expected top-level function {tool_name}");
        };
        assert_eq!(
            tool.parameters
                .properties
                ...
                .and_then(|message| message.encrypted),
            None,
            "portable message fields must remain plaintext"
        );
    }
    ...
    let native_spawn = namespace
        ...
        .expect("native spawn_agent spec");
    assert_eq!(
        native_spawn
            .parameters
            .properties
            ...
            .and_then(|message| message.encrypted),
        Some(true),
        "native collaboration messages must retain OpenAI encryption"
    );
    assert!(
        native_spawn
            .parameters
            .properties
            .as_ref()
            .is_some_and(|properties| !properties.contains_key("model_provider")),
        "portable tools must not alter the reserved spawn_agent schema"
    );
}
