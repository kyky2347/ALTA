...
#[test_case(None, true, false; "plaintext")]
#[test_case(None, false, true; "portable collaboration alias")]
...
async fn multi_agent_v2_spawn_sends_agent_message_to_child(
    ...
    let server = start_mock_server().await;
    let expected_plaintext = plaintext || portable_alias;
    let message = if expected_plaintext {
    ...
    let spawn_args = serde_json::to_string(&spawn_args)?;
    let mut spawn_event = if portable_alias {
        ev_function_call(SPAWN_CALL_ID, "alta_spawn_model_agent", &spawn_args)
    } else {
        ev_function_call_with_namespace(
            SPAWN_CALL_ID,
            MULTI_AGENT_V2_NAMESPACE,
        ...
        .with_config(move |config| {
            ...
            if portable_alias {
                config.agent_roles.insert(
                    "alta_test".to_string(),
                    AgentRoleConfig {
                        description: Some("ALTA portable role".to_string()),
                        config_file: None,
    ...
    let content = if expected_plaintext {
        vec![json!({
            "type": "input_text",
            "text": format!(
    ...
    if portable_alias {
        assert!(
            ...
            "portable cross-provider spawns must not inherit encrypted parent history"
        );
    }
    assert_eq!(
    ...
    let logged_message = if expected_plaintext {
        "[plaintext]"
    } else {
        message
    };
