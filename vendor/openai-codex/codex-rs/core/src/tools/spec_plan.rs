...
use crate::tools::handlers::multi_agents_v2::FOLLOWUP_MODEL_TASK_TOOL_NAME;
use crate::tools::handlers::multi_agents_v2::FollowupTaskHandler as FollowupTaskHandlerV2;
use crate::tools::handlers::multi_agents_v2::InterruptAgentHandler;
use crate::tools::handlers::multi_agents_v2::ListAgentsHandler as ListAgentsHandlerV2;
use crate::tools::handlers::multi_agents_v2::PortableCollaborationAlias;
use crate::tools::handlers::multi_agents_v2::SEND_MODEL_MESSAGE_TOOL_NAME;
use crate::tools::handlers::multi_agents_v2::SPAWN_MODEL_AGENT_TOOL_NAME;
...
const ALTA_AGENT_ROLE_PREFIX: &str = "alta_";
const ALTA_PORTABLE_SPAWN_GUIDANCE: &str = "ALTA cross-provider tool. Use this instead of spawn_agent when the selected agent_type uses a different model provider; it delivers the delegated task as portable plaintext in a clean task-only context. fork_turns is always forced to none so provider-private encrypted history cannot enter the child.";
const ALTA_PORTABLE_MESSAGE_GUIDANCE: &str = "ALTA cross-provider tool. Use this instead of send_message when the target agent uses a different model provider; it delivers the message as portable plaintext.";
const ALTA_PORTABLE_FOLLOWUP_GUIDANCE: &str = "ALTA cross-provider tool. Use this instead of followup_task when the target agent uses a different model provider; it delivers the follow-up as portable plaintext.";
    ...
    if collab_tools_enabled(turn_context) {
        if multi_agent_v2_enabled(turn_context) {
            ...
            if turn_context
                ...
                .agent_roles
                .keys()
                .any(|role| role.starts_with(ALTA_AGENT_ROLE_PREFIX))
            {
                registry.register_trusted_with_exposure(
                    Arc::new(PortableCollaborationAlias::new(
                        SpawnAgentHandlerV2::new(spawn_options),
                        SPAWN_MODEL_AGENT_TOOL_NAME,
                        ALTA_PORTABLE_SPAWN_GUIDANCE,
                    )),
                    exposure,
                );
                registry.register_trusted_with_exposure(
                    Arc::new(PortableCollaborationAlias::new(
                        ...
                        SEND_MODEL_MESSAGE_TOOL_NAME,
                        ALTA_PORTABLE_MESSAGE_GUIDANCE,
                    )),
                    exposure,
                );
                registry.register_trusted_with_exposure(
                    Arc::new(PortableCollaborationAlias::new(
                        ...
                        FOLLOWUP_MODEL_TASK_TOOL_NAME,
                        ALTA_PORTABLE_FOLLOWUP_GUIDANCE,
                    )),
                    exposure,
                );
            }
