...
> 执行只有两种模式：**Shadow** 和 **Broker API**。券商交易必须明确选择账户、
> 完成必要核验并单独授权。六家连接器已实现，但并非全部能够通过授权条件，
> 也不代表六家实盘账户已完成验收。
...
| 两种模式与明确的券商执行目标；保存选择不等于授权。                                                    | 按角色选择模型，保留独立评审。                                                                      |

研究截图：2026 年 9 月 12 日；券商流程：9 月 13 日。简体中文界面，Agent 原始产物保留撰写语言。
截图范围及哈希：[研究验收](docs/audits/console-readability-2026-09-12.md) ·
[券商流程验收](docs/audits/broker-routing-2026-09-13.md)。
不包含密钥或账户资料。
...
| **Broker API** | 已审计的计划发送至所选券商、明确配置的模拟盘或实盘账户，不自动改用 Tiger。 |
...
后端将持久化的交易表达和独立审计绑定到准确的账户配置版本；独立监控循环刷新报价、
对账和处理退出，不等待 LLM 研究结束。当前券商执行范围是单个活跃计划、做多美元股票或 ETF、
整股及当日有效限价单。选择券商、保存密钥都不会自动开启交易。

Tiger、Alpaca、IBKR、Futu 已有满足核验条件后可执行的路径；IBKR 每次下单前还需无成交的预检。
**Longbridge 的身份核验，以及 Schwab 的权限、完整订单核验和 OAuth 续期尚未完整实现**，
因此这两家的自动下单授权仍被阻止。六家连接器不等于六家实盘验收通过。
旧 Tiger 模拟盘执行器仅作隔离兼容保留，不是第三个可选模式。

首次授权需要空的专用账户；切换前须撤销授权并对账、处理已有仓位。
请保持后端运行以管理退出：停止服务**不会**清空券商持仓，目前也未实现券商原生保护单。
[供应商矩阵、配置与限制 →](docs/broker-expansion.md)
