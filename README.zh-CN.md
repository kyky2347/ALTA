...
**最新本地验证状态：** 189 项 Node/harness、366 项 Opportunity OS、38 项隔离
...
**本地开发状态：** 已接入机构化组合智能和可重放的市场研究漏斗。系统现在最多使用 24 个完整
交易日，将当前变化和标的自身的稳健历史波动比较，拆分隔夜与日内价格发现，并定位没有单日标题式
跳涨、但持续偏离基准的安静预期漂移。完整日线最多给每个 Trader Mind 分配一个非 Evidence
异常问题，Mind 必须重新核验行情、公司行动、数据质量、波动制度、因子暴露、因果路径和反方解释；
归一化分数本身永远不是证据或交易信号。系统还会用
...
持有受管模拟盘组合时撤权会进入只允许平仓的 drain：禁止新 BUY，但允许每个持久化退出完成，避免把
...
flowchart TB
  subgraph discover["发现"]
    ...
    evidence --> screen["完整日线研究漏斗<br/>稳健异常 · 持续漂移 · 隔夜/日内 · 市场宽度"]
  ...
  registry --> debate
  allocation --> shadow
  shadow -. 操作员授权后 .-> paper["隔离的 Tiger 模拟盘镜像<br/>Agent 自主提出风险仓位 · DAY 限价 · 新鲜提交报价"]
...
| 经纪账户边界   | 默认禁用；generation 授权、脱敏资产/持仓/订单、账户与配置双重绑定、持久化 Paper intent/outbox、稳定券商历史标记和撤权 close-only drain；Agent 可提出数量，但确定性风险、流动性、券商容量、报价新鲜度和名义金额边界会把它裁剪成一笔精确的正整股 DAY 限价模拟单；按用户/账户隔离且跨 checkout 共享的私有租约串行化变更，不存在实盘或浏览器手工下单路径 |
| 真实世界 Alpha | **尚未建立**；承销校准仍属探索性，仍需积累更大规模、计入成本的独立前向 Shadow 样本                                                                                                                                                                                                                                                                   |

### 最新本地验收

当前本地工作树通过了 189 项 Node/harness 测试、366 项 Opportunity OS Python 测试、
38 项隔离 capital 包测试和 9 项前端选择逻辑测试。2026-08-27 08:19–17:00 EDT 的受监督真实墙钟试运行观察到 28 个
...
flowchart TB
  ...
  shadow -. "显式验收" .-> paper["Tiger 模拟盘镜像<br/>Agent 请求仓位 · 硬风控裁剪<br/>精确多股监控、退出与核验"]
...
追价上限、超时取消和禁止自动改价。Tiger 模拟盘镜像现在使用 Agent 明确提出、再经确定性风险与
券商门槛裁剪的多股仓位；Agent 仍然没有下单工具，
系统也没有任何实盘账户路径。
...
| 验证项目                         |                                                        结果 |
| -------------------------------- | ----------------------------------------------------------: |
| Node / Opportunity OS / 资金边界 |                                 189 / 366 / 38 tests passed |
...
- Tiger 变更能力默认禁用，只存在于隔离的模拟盘执行器中。它以 SHA-256 精确绑定一个
  17 位模拟盘账户，拒绝实盘与 sandbox 配置，只允许经过 Agent 仓位请求和确定性组合风险裁剪的
  正整数股 DAY 限价单；提交前还要求 10 秒内的新鲜报价、券商可交易数量和订单预览通过，并使用
  持久化 intent/outbox 和近期订单标记做 fail-closed 对账。Tiger 没有向 ALTA 提供服务端幂等保证；任何无法唯一证明的
  状态都要求人工审查，绝不会自动重复下单。
...
generation；空仓时可完全撤权，已有受管仓位时则进入 recovery-required 的 close-only generation，
禁止开仓并允许持久化退出完成后再彻底禁用。授权后，下一次启动只能把已经通过研究、风险和审计门槛的股票表达镜像到隔离的模拟盘执行器，
并继续受风险仓位上限、DAY 限价单、新鲜报价和正常交易时段限制；Agent 本身仍然拿不到券商凭据或下单工具，且系统
不存在任何实盘通道。后续已授权重启只有在券商无挂单、每个 STK 的精确数量都与开放持久化 ALTA
持仓完全一致、且最新持久化安全事件证明对应 BUY 由 ALTA 成交时，才允许保留该模拟盘组合；孤儿持仓、
