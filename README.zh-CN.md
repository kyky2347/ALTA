...
Follow-up 的父 Opportunity、精确问题和 lineage 必须完整装入生产 Prompt 预算，否则显式失败，
不能再静默退化成泛化探索。SEC 研究还可通过官方 ticker 映射直接解析 CIK，再读取
...
_这是当前构建的合成操作台：展示机会流、持久化 Agent 交接、委员会状态、审计表达和事件回放，
...
_这是当前构建的合成前向证据页：同时展示选择偏差调整、预测误差、组合集中度、执行质量和资金姿态，
不会把 Shadow 结果写成已证明的 Alpha。_

[![ALTA 合成凭据中心中的只写 Provider 激活与脱敏 API 健康状态](docs/assets/alta-credential-center.png)](docs/assets/alta-credential-center.png)

_这是当前构建版本的凭据中心截图，只使用合成状态演示验证成功、限流和鉴权拒绝；不包含真实
Provider 状态、指纹、Token、账户标识或经纪数据。三张图片都来自禁用控制功能的 `?preview=1`
界面。_
...
| 证据与状态     | 时点 Evidence、不可改写的论点支柱、全局机会连续性、固定决策期限、受限问题与过程记忆、多维研究注意力组合、建仓时冻结的研究归因、成熟度门控的 Mind Alpha 反馈、可撤销的只读研究激励、Replay                               |
...
| 无人值守服务   | 内部调度器、单主租约、按机会积压调整节奏、统一上下文预算、数据源退避、开放持仓更快观察、runtime 重建、只传字段路径/错误码且更新持久期限的可审计 Scout 重试、健康看门狗和安全退出                                        |
| 只读可观测性   | Runtime、Run、Opportunity、冻结 cohort、缺失率、MFE/MAE/回撤/退出捕获和 Shadow 测量的 loopback JSON 与 SSE 接口                                                                                                         |
| 经纪账户边界   | 默认禁用；本机认证开关、脱敏资产/持仓/订单、账户与配置双重绑定，并仅允许隔离执行器对一个精确 Tiger 模拟盘账户执行 1 股限价单；不存在实盘或浏览器手工下单路径                                                            |
| 真实世界 Alpha | **尚未建立**；承销校准仍属探索性，仍需积累更大规模、计入成本的独立前向 Shadow 样本                                                                                                                                      |

### 最新本地验收

当前本地工作树通过了 183 项 Node/harness 测试、313 项 Opportunity OS Python 测试和
27 项隔离 capital 包测试。2026-08-27 08:19–17:00 EDT 的受监督真实墙钟试运行观察到 28 个
...
| 验证项目                         |                                                        结果 |
| -------------------------------- | ----------------------------------------------------------: |
| Node / Opportunity OS / 资金边界 |                                 183 / 313 / 27 tests passed |
...
随后才持久化授权。配置文件一旦变化，旧授权自动失效。关闭时先耐久撤权，再安全停止正在运行的服务
和依赖。授权后，下一次启动只能把已经通过研究、风险和审计门槛的股票表达镜像到隔离的模拟盘执行器，
