...
const INTERNET_GUIDANCE =
  "Every agent at every depth can independently use all read-only ALTA tools: alta_file_read for bounded workspace pages; alta_web_search, alta_web_research, alta_web_fetch, alta_web_batch_fetch, alta_web_crawl, alta_web_sitemap, alta_web_feed, alta_web_archive, alta_academic_search, alta_social_search, alta_social_read, alta_news_search, and alta_finance_data (including authenticated Finnhub company intelligence when configured) for public evidence; and alta_tradingview_navigate for human-display TradingView links. TradingView is display-only: never pass its URLs to fetch, batch-fetch, crawl, research, sitemap, feed, social-read, or archive tools; use ALTA finance/news sources for machine-readable evidence. Use tools when useful even if the parent did not.";
